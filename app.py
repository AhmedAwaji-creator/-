...
def _dark_alert(msg, bg, border, icon):
    st.markdown(
        ...
        f"padding:12px 16px;direction:rtl;font-size:14px;color:#ddeeff;margin:6px 0;'>"
        f"{icon} {msg}</div>",
...
def render_invoice_html(title, items_list, warehouse, contractor, employee, custom_inv_no=None, extra_info=None, boq=""):
    ...
    for item in items_list:
        rows += (
            "<tr>"
            f"<td style='border:1px solid rgba(0,140,255,0.15);padding:10px;text-align:center;font-weight:bold;'>{item['code']}</td>"
            f"<td style='border:1px solid rgba(0,140,255,0.15);padding:10px;text-align:right;'>{item['name']}</td>"
            f"<td style='border:1px solid rgba(0,140,255,0.15);padding:10px;text-align:center;color:#7aaac8;font-weight:bold;font-size:16px;'>{item['qty']}</td>"
            "</tr>"
        )
    extra_row = ""
    if extra_info:
    ...
    @media print {{
        ...
        body {{ margin: 0mm; padding: 10mm; background:rgba(3,10,28,0.82); }}
        .no-print {{ display: none !important; }}
    }}
    </style>
    <div dir="rtl" style="font-family:'Tajawal',Arial,sans-serif;padding:30px;border:3px solid #004a99;border-radius:15px;background:rgba(3,10,28,0.82);max-width:900px;margin:auto;">
        <div style="text-align:center;margin-bottom:15px;">
            <img src="{LOGO_DATA_URI}" width="160" style="border-radius:10px;display:block;margin:0 auto 8px auto;">
            <div style="font-size:22px;font-weight:900;color:#7aaac8;">السعودية للطاقة</div>
            <div style="font-size:13px;color:#1dda70;font-weight:bold;">نظام إدارة مواد الطوارئ — دائرة شرق منطقة جازان</div>
        </div>
        <hr style="border:2px solid #004a99;margin-bottom:12px;">
        <div style="display:flex;justify-content:space-between;font-size:14px;color:#c8dff4;margin-bottom:12px;">
            <div>
        ...
        <div style="background:rgba(3,10,28,0.75);padding:12px 15px;border-radius:8px;margin-bottom:18px;border:1px solid rgba(0,140,255,0.15);font-size:14px;">
...
def page_header(icon, title, subtitle="", accent="#004a99"):
    ...
    ">
        ...
        {"<div style='font-size:13px;color:#6898c0;margin-top:5px;'>" + subtitle + "</div>" if subtitle else ""}
    </div>
    """, unsafe_allow_html=True)
        ...
        if warehouse and conn is not None:
            try:
                avail = int(pd.read_sql(
                    ...
                    conn).iloc[0]['t'])
                avail_txt = f"<span style='color:#6898c0;font-size:11px;'> / {avail}</span>"
            except Exception:
                pass
        rows += f"""
        <tr>
            <td style='padding:8px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-weight:900;color:#7aaac8;font-size:12px;'>{item['code']}</td>
            <td style='padding:8px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:13px;'>{item['name']}</td>
            <td style='padding:8px 12px;border-bottom:1px solid rgba(0,140,255,0.10);color:#7aaac8;font-size:12px;'>{item.get('cat','—')}</td>
...
def html_table(df, accent="#004a99", info_label=None, badge_col=None,
    ...
    for _, row in df.iterrows():
        ...
        for col in cols:
            ...
            # الأرقام تُعرض وسطاً
                ...
                cell = f"<td style='padding:8px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:13px;'>{val}</td>"
            cells += cell
    ...
    if info_label:
        info_bar = f"""<div style='display:flex;justify-content:space-between;align-items:center;
            padding:8px 14px;background:rgba(0,20,60,0.60);border-bottom:1px solid #dde6f5;
            font-size:13px;color:#8aaac8;direction:rtl;'>
...
st.markdown("""
    ...
    .stTextArea > div > div > textarea::placeholder {
        color: rgba(180,210,240,0.40) !important;
    }
    ...
    .sb-btn-invoices > div > button {
        ...
        font-size: 13px !important; color: #c8dff4 !important;
    ...
    .ret-btn-wrap .ret-badge {
        ...
        min-width: 20px; height: 20px; font-size: 11px; font-weight: 900;
    ...
    .inv-row {
        ...
        border-bottom: 1px solid rgba(0,140,255,0.10); font-size: 13px;
    ...
    [data-baseweb="calendar"] [aria-disabled="true"] {
        color: rgba(100,150,200,0.30) !important;
    }
    /* Today highlight */
...
if _qp_page:
    # دائماً نستعيد الصفحة من الـ URL عند وجودها
    st.session_state.page = _qp_page

# ── استعادة الجلسة من query param عند تحديث المتصفح (بدون تسجيل خروج) ──
...
(function(){
    ...
    // ══════════════════════════════════════════════════
    // ٢. حفظ المستخدم في localStorage واستعادة الجلسة
    // ══════════════════════════════════════════════════
...
(function(){
  try {
    ...
    function applyDark(){
      ...
      // كل العناصر ذات خلفية بيضاء أو فاتحة
      ...
      style.textContent = [
        ...
        '[data-baseweb="calendar"],[data-baseweb="datepicker"],[data-baseweb="stateful-calendar"]{background:rgba(3,10,28,0.97)!important;color:#c8dff4!important;border-color:rgba(0,140,255,0.25)!important;}',
...
# =========================================================
    ...
    with st.sidebar:
        ...
        '>
            ...
            <div style='font-size:15px;font-weight:900;color:#ddeeff;margin-bottom:3px;'>
                نظام إدارة مواد الطوارئ
            </div>
            <div style='font-size:12px;font-weight:700;color:#1dda70;margin-bottom:10px;'>
            ...
            <div style='
                ...
                margin-bottom:10px;
            '></div>
            <div style='font-size:13px;color:#7aaac8;margin-bottom:5px;'>الموظف</div>
            <div style='font-size:15px;font-weight:900;color:#ddeeff;margin-bottom:7px;'>
                {u['full_name']}
            </div>
            <span style='
                background: linear-gradient(135deg,rgba(0,100,220,0.50),rgba(29,170,96,0.30));
                border: 1.5px solid rgba(0,180,255,0.55);
                color: #ddeeff; font-size:13px; font-weight:900;
...
""", height=90, scrolling=False)
        ...
        if role == "موجه بلاغات":
            ...
            st.divider()
            st.sidebar.markdown("<div class='sb-section-stock'>📦 عمليات الصرف والارجاع</div>", unsafe_allow_html=True)
            st.markdown("<div class='sb-btn-stock'>", unsafe_allow_html=True)
            if st.button("🛒 صرف مواد للمقاول", key="user_stockout"): st.session_state.page = "stock_out"; st.query_params["_pg"] = "stock_out"
        ...
        # ── مسؤول المستودعات: كل شيء إلا التحكم بالنظام ──
            ...
            if st.button("📄 فواتيري", key="sb_my_inv_wh"): st.session_state.page = "my_invoices"; st.query_params["_pg"] = "my_invoices"
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        # ── أمين مستودع المقاول: صلاحيات محدودة ──
            ...
            if _pend_total > 0:
                ...
                <div style='background:rgba(80,50,0,0.45);border:2px solid #f9a825;border-radius:10px;
                    ...
                    ⏳ <b style='color:#ffaa66;font-size:15px;'>{_pend_total}</b>
                    <span style='font-size:12px;color:#8aaac8;'> فاتورة بانتظار إرفاقك</span>
        ...
        # ── مدير النظام: صلاحيات كاملة ──
            ...
            if st.button("📄 فواتيري", key="sb_my_inv_adm"): st.session_state.page = "my_invoices"; st.query_params["_pg"] = "my_invoices"
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if role == "مدير نظام":
            ...
            if not _all_users_sw.empty:
                st.divider()
                st.markdown(
                    "<div style='font-size:12px;font-weight:700;color:#6898c0;"
                ...
                if _sw_uname and _sw_uname != u['username']:
                    ...
                    st.markdown(
                        ...
                        f"border-radius:8px;padding:8px 12px;font-size:12px;direction:rtl;"
                        f"margin-bottom:8px;'>"
        ...
        if role == "مدير نظام" or user_mobile_access == 1:
            st.divider()
            st.markdown("""
            <p style='text-align:center; font-weight:bold; color:#8aaac8; margin:0 0 6px 0; font-size:13px;'>
                🖥️ اختر نسخة العرض
        ...
        '>
            <div style='
                ...
                margin-bottom:10px;
            '></div>
            <div style='font-size:12px;color:#4a7a9a;margin-bottom:4px;'>تطوير</div>
            <div style='font-size:13px;font-weight:900;color:#a8ccee;margin-bottom:6px;'>
                أحمد سعيد عواجي
            </div>
    ...
    if st.session_state.get("logout_confirm"):
        ...
        with col_card:
            ...
            <div style='background:rgba(3,10,28,0.82);border:2px solid #e53e3e;border-radius:20px;
                ...
                <div style='font-size:14px;color:#7aaac8;margin-bottom:28px;line-height:1.7;'>
    ...
    if st.session_state.page == "inventory_status":
        ...
        if not df_inventory.empty:
            ...
            for _, row in df_inventory.iterrows():
                ...
                rows_html += f"""
                <tr>
                    <td style='font-weight:900;color:#7aaac8;font-size:12px;padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);'>{row['كود المادة']}</td>
                    <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:13px;'>{row['اسم الصنف والمادة التفصيلي']}</td>
                    <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);color:#8aaac8;font-size:12px;'>{row['تصنيف الفئة']}</td>
                    <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:12px;font-weight:bold;color:#c8dff4;'>📍 {row['موقع المستودع']}</td>
                    <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);text-align:center;'>
            ...
            <style>
                ...
                .info-bar {{ display:flex; justify-content:space-between; align-items:center;
                             padding:8px 12px; background:rgba(0,20,60,0.60); border-bottom:1px solid #dde6f5;
                             font-size:13px; color:#8aaac8; direction:rtl; }}
                .legend {{ font-size:11px; color:#6898c0; padding:6px 12px; text-align:right; background:#fafbff; }}
            </style>
            </head><body>
            <div class="info-bar">
                <span>📦 إجمالي الأصناف المعروضة: <b style="color:#7aaac8;font-size:15px;">{len(df_inventory)}</b></span>
            ...
            with st.form("multi_material_define_form", clear_on_submit=True):
                ...
                for i in range(st.session_state.num_materials_rows):
                    st.markdown(f"""<div style='background:rgba(0,20,60,0.60);border-right:3px solid #004a99;border-radius:6px;
                        padding:6px 14px;margin:6px 0 4px 0;font-weight:bold;color:#7aaac8;font-size:13px;'>
        ...
        if not df_alert_data.empty:
            ...
            if not df_to_export.empty:
                ...
                for _, row in df_to_export.iterrows():
                    ...
                    if stat == "🔴 نقص حاد":
                        badge_bg = "#d32f2f"; badge_color = "white"
                        row_bg   = "background:rgba(60,0,0,0.40);"
                    elif stat == "🟡 تنبيه":
                        badge_bg = "#f9a825"; badge_color = "#333"
                        row_bg   = "background:rgba(60,50,0,0.40);"
                    else:
                    ...
                    <tr style='{row_bg}'>
                        <td style='font-weight:900;color:#7aaac8;font-size:12px;padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);'>{row['كود المادة']}</td>
                        <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:13px;'>{row['اسم المادة والصنف']}</td>
                        <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);color:#8aaac8;font-size:12px;'>{row['الفئة الأساسية']}</td>
                        <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:12px;font-weight:bold;color:#c8dff4;'>📍 {row['المستودع المستضيف']}</td>
                        <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);text-align:center;'>
                            <span style='display:inline-block;background:{badge_bg};color:{badge_color};border-radius:20px;padding:3px 14px;font-weight:900;font-size:13px;min-width:36px;text-align:center;'>{qty}</span>
                        </td>
                        <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);text-align:center;'>
                ...
                <style>
                    ...
                    thead th {{ padding:11px 12px; text-align:right; font-weight:900; letter-spacing:0.3px; }}
                    tbody tr:nth-child(even) {{ background:rgba(3,10,28,0.70); }}
                    ...
                    .info-bar {{ display:flex; justify-content:space-between; align-items:center;
                                 ...
                                 font-size:13px; color:#8aaac8; direction:rtl; }}
                    .legend {{ font-size:11px; color:#6898c0; }}
                </style>
                </head><body>
                <div class="info-bar">
                    <span>⚠️ الأصناف التي تحتاج تدخلاً: <b style="color:#ff6666;font-size:15px;">{len(df_to_export)}</b> صنف</span>
    ...
    # ---------------------------------------------------------
            ...
            with st.form("stock_in_form", clear_on_submit=True):
                ...
                for ri in range(st.session_state.num_in_rows):
                    st.markdown(f"""<div style='background:rgba(0,50,20,0.35);border-right:3px solid #1daa60;border-radius:6px;
                        padding:5px 14px;margin:6px 0 3px 0;font-weight:bold;color:#1dda70;font-size:13px;'>
        ...
        if st.session_state.get('stock_in_pending'):
            ...
            st.write("---")
            _si_rows_html = "".join([
                f"<tr><td style='padding:9px 14px;border-bottom:1px solid #c8e6c9;text-align:center;font-weight:bold;color:#1dda70;'>{rv['code']}</td>"
                f"<td style='padding:9px 14px;border-bottom:1px solid #c8e6c9;'>{rv['name']}</td>"
                f"<td style='padding:9px 14px;border-bottom:1px solid #c8e6c9;text-align:center;'><b style='color:#1dda70;'>{rv['wh']}</b></td>"
                f"<td style='padding:9px 14px;border-bottom:1px solid #c8e6c9;text-align:center;font-weight:900;font-size:16px;color:#1dda70;'>{rv['qty']}</td></tr>"
                for rv in pending_rows
            ])
            st.markdown(f"""
            <div dir="rtl" style="background:linear-gradient(135deg,#f1f8e9,#e8f5e9);
                ...
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
                    ...
                    <div>
                        ...
                        <div style="font-size:13px;color:#6898c0;margin-top:3px;">سيتم إضافة المواد التالية للمستودعات</div>
                    </div>
                </div>
    ...
    # ---------------------------------------------------------
        ...
        if not list_warehouses or not list_contractors:
                    ...
                    if out_code:
                        ...
                        if mat.empty:
                            ...
                            if avail < (out_qty + already):
                                ...
                                if ex:
                                    st.warning(f"⚠️ هذه المادة [{out_code}] مضافة مسبقاً — سيتم تجميع الكمية.")
                                    st.session_state.cart[ex[0]]["qty"] += out_qty
                                else:
                                    st.session_state.cart.append({"code": out_code, "name": mat.iloc[0]["item_name"], "qty": out_qty, "cat": mat.iloc[0]["category"]})
            ...
            # ════════════════════════════════════════════
                ...
                rows_html = "".join([
                    f"<tr><td style='padding:9px 14px;border-bottom:1px solid #ffeeba;text-align:center;font-weight:bold;color:#ff6666;'>{it['code']}</td>"
                    f"<td style='padding:9px 14px;border-bottom:1px solid #ffeeba;'>{it['name']}</td>"
                    f"<td style='padding:9px 14px;border-bottom:1px solid #ffeeba;text-align:center;font-weight:900;font-size:16px;color:#ff6666;'>{it['qty']}</td></tr>"
                    for it in st.session_state.cart
                ])
                _boq_line = f"<div style='background:rgba(80,50,0,0.45);border-right:4px solid #f9a825;border-radius:6px;padding:8px 14px;margin-top:12px;font-size:13px;'>📋 <b>BOQ:</b> <span style='color:#ffaa66;font-weight:bold;'>{_final_boq}</span></div>" if _final_boq else ""
                st.markdown(f"""
                <div dir="rtl" style="background:linear-gradient(135deg,rgba(50,35,0,0.60),rgba(45,32,0,0.50));
                    border:3px solid #f9a825;border-radius:16px;
                    padding:24px 28px;box-shadow:0 4px 20px rgba(249,168,37,0.25);margin-bottom:16px;">
                    <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
                        <span style="font-size:36px;">⚠️</span>
                        <div>
                            <div style="font-size:20px;font-weight:900;color:#ffaa66;">تأكيد إصدار فاتورة صرف</div>
                            <div style="font-size:13px;color:#6898c0;margin-top:3px;">يرجى مراجعة التفاصيل قبل التأكيد النهائي</div>
                        </div>
                    </div>
                    <div style="background:rgba(3,10,28,0.82);border-radius:10px;padding:12px 16px;margin-bottom:12px;font-size:14px;line-height:2.2;direction:rtl;">
    ...
    # ---------------------------------------------------------
        ...
        if not list_warehouses or not list_contractors:
            ...
            if not st.session_state.confirm_return:
                ...
                rows_html = "".join([
                    f"<tr><td style='padding:9px 14px;border-bottom:1px solid #c8e6c9;text-align:center;font-weight:bold;color:#1dda70;'>{it['code']}</td>"
                    f"<td style='padding:9px 14px;border-bottom:1px solid #c8e6c9;'>{it['name']}</td>"
                    f"<td style='padding:9px 14px;border-bottom:1px solid #c8e6c9;text-align:center;font-weight:900;font-size:16px;color:#1dda70;'>{it['qty']}</td></tr>"
                    for it in st.session_state.return_cart
                ])
                _ret_boq_line = f"<div style='background:rgba(0,50,20,0.45);border-right:4px solid #1daa60;border-radius:6px;padding:8px 14px;margin-top:12px;font-size:13px;'>📋 <b>BOQ:</b> <span style='color:#1dda70;font-weight:bold;'>{ret_boq}</span></div>" if ret_boq else ""
                st.markdown(f"""
                <div dir="rtl" style="background:linear-gradient(135deg,#f1f8e9,#e8f5e9);
                    border:3px solid #1daa60;border-radius:16px;
                    padding:24px 28px;box-shadow:0 4px 20px rgba(29,170,96,0.2);margin-bottom:16px;">
                ...
                if not _oldest_rr.empty and _oldest_rr.iloc[0]['oldest']:
                    try:
                        ...
                        st.markdown(
                            ...
                            f"padding:10px 16px;direction:rtl;font-size:13px;margin-bottom:14px;"
                ...
                if df_my_req.empty:
                    ...
                    for _, rr in df_my_req.iterrows():
                        ...
                        st.markdown(
                            ...
                            f" <span style='background:{status_color};color:white;border-radius:8px;padding:2px 10px;font-size:13px;'>{rr['status']}</span><br>"
                    ...
                    for _, rr in df_pending.iterrows():
                        ...
                        with st.expander(f"📄 طلب رقم {rr['request_no']} | من: {rr['requester']} | {rr['timestamp']}", expanded=True):
                            st.markdown(
                                ...
                                + (f"<br>📄 <b>رقم الفاتورة الأصلية:</b> <span style='color:#ff6666;font-weight:bold;font-size:15px;'>{str(rr.get('original_invoice_no','') or '—')}</span>" )
        ...
        if not list_warehouses:
            ...
            if not st.session_state.confirm_transfer:
                ...
                rows_html = "".join([
                    ...
                    f"<td style='padding:9px 14px;border-bottom:1px solid #bbdefb;text-align:center;font-weight:900;font-size:16px;color:#7aaac8;'>{it['qty']}</td></tr>"
                ...
                <div dir="rtl" style="background:linear-gradient(135deg,rgba(0,25,70,0.60),rgba(0,20,60,0.50));
                    ...
                    <div style="background:rgba(3,10,28,0.82);border-radius:10px;padding:12px 16px;margin-bottom:12px;font-size:14px;direction:rtl;">
                        <div style="display:flex;align-items:center;justify-content:center;gap:16px;flex-wrap:wrap;">
                            <div style="text-align:center;background:rgba(80,0,0,0.40);border-radius:8px;padding:10px 20px;">
                                <div style="font-size:11px;color:#6898c0;margin-bottom:4px;">📤 من مستودع</div>
                                <div style="font-size:16px;font-weight:900;color:#ff6666;">{trans_wh_from}</div>
                            </div>
                            ...
                            <div style="text-align:center;background:rgba(0,50,20,0.45);border-radius:8px;padding:10px 20px;">
                                <div style="font-size:11px;color:#6898c0;margin-bottom:4px;">📥 إلى مستودع</div>
                                <div style="font-size:16px;font-weight:900;color:#1dda70;">{trans_wh_to}</div>
                            </div>
                        </div>
                        <div style="text-align:center;margin-top:10px;font-size:13px;color:#8aaac8;">
    ...
    # ---------------------------------------------------------
        ...
        if _direct_inv_no:
            ...
            if not _direct_row.empty:
                ...
                st.markdown(
                    ...
                    + "</div>", unsafe_allow_html=True)
                components.html(str(_dr.get('html_content','')), height=540, scrolling=True)
        ...
        if _latest_p:
            st.markdown("""<div style='display:flex;align-items:center;gap:10px;margin:12px 0 16px 0;direction:rtl;'>
                ...
                <span style='background:#004a99;color:white;border-radius:20px;padding:6px 20px;font-size:14px;font-weight:900;'>🆕 آخر فاتورة</span>
        ...
        st.markdown("""<div style='display:flex;align-items:center;gap:10px;margin:22px 0 16px 0;direction:rtl;'>
            ...
            <span style='background:#555;color:white;border-radius:20px;padding:6px 20px;font-size:14px;font-weight:900;'>📋 جميع الفواتير السابقة</span>
        ...
        if df_p.empty:
            ...
            for _,_pr in df_p.iterrows():
                ...
                st.markdown(
                    ...
                    f"padding:10px 14px;direction:rtl;font-size:13px;'>"
                    f"<span style='background:{_ptc};color:white;border-radius:4px;padding:1px 6px;font-size:11px;'>{str(_pr.get('invoice_type',''))}</span>"
                ...
                if st.session_state.view_archived_html.get(_pr['invoice_no'], False):
                    components.html(_pr['html_content'], height=520, scrolling=True)
                st.markdown("<hr style='margin:4px 0;'>", unsafe_allow_html=True)
    ...
    # ---------------------------------------------------------
        ...
        with _mi_tab1:
            ...
            if df_mf.empty:
                ...
                for _,mrow in df_mf.iterrows():
                    ...
                    st.markdown(
                        f"<div style='background:rgba(3,10,28,0.70);border-right:4px solid {_mt_c};border-radius:8px;padding:10px 14px;direction:rtl;font-size:13px;'>"
                        f"<span style='background:{_mt_c};color:white;border-radius:4px;padding:1px 6px;font-size:11px;'>{str(mrow['invoice_type'])}</span>"
        ...
        with _mi_tab2:
            ...
            _stat_filter = _f2.selectbox("📊 الحالة:", ["الكل","📝 تحتاج إرفاق","🔄 بانتظار الاعتماد","🔴 مُعادة","✅ معتمدة"], key="cwk_stat_filter2")
            ...
            # ── إحصائية شاملة ──
            _tp = len(df_unsigned)
            _tw = len(df_signed[df_signed['status']=='بانتظار الاعتماد']) if not df_signed.empty else 0
            _tr = len(df_signed[df_signed['status']=='مُعادة'])            if not df_signed.empty else 0
            _ta = len(df_signed[df_signed['status']=='معتمد'])             if not df_signed.empty else 0

            st.markdown(f"""
            <div style='display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:16px 0;direction:rtl;'>
                <div style='background:rgba(60,0,0,0.40);border:2px solid #e53e3e;border-radius:12px;padding:16px;text-align:center;'>
                ...
                <div style='background:rgba(0,30,80,0.50);border:2px solid #0288d1;border-radius:12px;padding:16px;text-align:center;'>
                    <div style='font-size:28px;font-weight:900;color:#4db8ff;'>{_tw}</div>
                    <div style='font-size:12px;color:#8aaac8;margin-top:4px;'>🔄 بانتظار الاعتماد</div>
                </div>
                <div style='background:rgba(60,20,0,0.45);border:2px solid #e65100;border-radius:12px;padding:16px;text-align:center;'>
                ...
                <div style='background:rgba(0,50,20,0.45);border:2px solid #1daa60;border-radius:12px;padding:16px;text-align:center;'>
            ...
            if _stat_filter in ("الكل","📝 تحتاج إرفاق"):
                for _, r in df_unsigned.iterrows():
                    rows.append({
                        "status":"📝 إرفاق","status_color":"#e53e3e","bg":"#fff5f5",
            ...
            # الفواتير المُرفقة
            stat_map = {
                "🔄 بانتظار الاعتماد": ("بانتظار الاعتماد","🔄 اعتماد","#0288d1","#e3f2fd"),
                "🔴 مُعادة":            ("مُعادة","🔴 مُعادة","#e65100","#fff5f0"),
                "✅ معتمدة":            ("معتمد","✅ معتمدة","#1daa60","#e8f5e9"),
            }
            ...
            if not rows:
                ...
                for _con, _con_rows in _by_con.items():
                    ...
                    # عنوان المقاول
                    _badges = ""
                    if _con_tp: _badges += f"<span style='background:#e53e3e;color:white;border-radius:20px;padding:2px 10px;font-size:11px;margin-right:4px;'>📝 {_con_tp}</span>"
                    if _con_tw: _badges += f"<span style='background:#0288d1;color:white;border-radius:20px;padding:2px 10px;font-size:11px;margin-right:4px;'>🔄 {_con_tw}</span>"
                    if _con_tr: _badges += f"<span style='background:#e65100;color:white;border-radius:20px;padding:2px 10px;font-size:11px;margin-right:4px;'>🔴 {_con_tr}</span>"
                    if _con_ta: _badges += f"<span style='background:#1daa60;color:white;border-radius:20px;padding:2px 10px;font-size:11px;'>✅ {_con_ta}</span>"
                    ...
                    with st.expander(f"🏗️ {_con}", expanded=_has_urgent):
                        ...
                        for _row in _con_rows:
                            ...
                            _row_html = (
                                ...
                                "font-size:13px;margin-bottom:8px;display:flex;"
                                "justify-content:space-between;align-items:center;'>"
                                "<div>"
                                f"<span style='background:{_tc};color:white;border-radius:4px;"
                                f"padding:1px 8px;font-size:11px;font-weight:bold;'>{_row['invoice_type']}</span>"
                                f"&nbsp;&nbsp;<b style='font-size:14px;'>{_row['invoice_no']}</b>"
                                f"&nbsp;<span style='background:{_row['status_color']};color:white;"
                                f"border-radius:10px;padding:1px 8px;font-size:11px;'>{_row['status']}</span>"
                                f"<br><span style='color:#7aaac8;font-size:12px;'>"
                                f"📍 {_row['warehouse']} &nbsp;|&nbsp; 👤 {_row['by']}"
                                f"</span>"
                                "</div>"
                                f"<span style='color:#6898c0;font-size:12px;white-space:nowrap;'>📅 {_row['date']}</span>"
                                "</div>"
                            )
                ...
                try:
                    ...
                    <div style='background:rgba(3,10,28,0.70);border:1px solid rgba(0,140,255,0.18);border-radius:12px;
                        ...
                        <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;'>
                            <span style='font-size:14px;font-weight:700;color:#c8dff4;'>
                ...
                for chunk in [grp_pos.iloc[i:i+cols_r] for i in range(0, len(grp_pos), cols_r)]:
                    ...
                    for ci, (_, ct) in enumerate(chunk.iterrows()):
                        with card_cols[ci]:
                            ...
                            nt = f"<div style='font-size:11px;color:#5a8aaa;margin-top:5px;'>📝 {ct['notes']}</div>" if ct.get('notes') else ""
                            st.markdown(f"""
            ...
            if df_con.empty:
                ...
                for con_name in sorted(contractors_shown):
                    ...
                    <div style='background:#fff3e0;border:2px solid #e65100;border-radius:12px;
                        ...
                        <span style='font-size:20px;font-weight:900;color:#ffaa66;'>🏗️ {con_name}</span>
                        <span style='font-size:12px;color:#aaa;background:rgba(3,10,28,0.82);border-radius:8px;
        ...
        with tab_warehouse:
            ...
            if df_wh.empty:
                ...
                for wh_name in sorted(warehouses_shown):
                    ...
                    st.markdown(f"""
                    <div style='background:rgba(0,20,60,0.50);border:2px solid #1a237e;border-radius:12px;
                                padding:12px 20px;margin:20px 0 6px 0;direction:rtl;'>
                    ...
                    for chunk in [grp_d.iloc[i:i+cols_m] for i in range(0, len(grp_d), cols_m)]:
                        ...
                        for ci, (_, mgr) in enumerate(chunk.iterrows()):
                            with mcols[ci]:
                                ...
                                nt_m = f"<div style='font-size:11px;color:#5a8aaa;margin-top:5px;'>📝 {mgr['notes']}</div>" if mgr.get('notes') else ""
                                st.markdown(f"""
                                <div style='background:rgba(3,10,28,0.82);border:2px solid #6a1b9a;border-radius:12px;
                                            padding:16px 10px;text-align:center;margin-bottom:10px;
                                            box-shadow:0 2px 8px rgba(0,0,0,0.07);'>
    ...
    # ---------------------------------------------------------
        ...
        if u['role'] != "مدير نظام":
            ...
            with tab_mobile_access:
                ...
                st.markdown("""
                <div style='background:rgba(0,30,80,0.50);border:2px solid #0288d1;border-radius:10px;padding:14px 18px;margin-bottom:18px;direction:rtl;font-size:14px;'>
                ...
                if df_mobile_users.empty:
                    ...
                    for idx_m, r_mob in df_mobile_users.iterrows():
                        ...
                        if is_admin:
                            col_m3.markdown("<span style='color:#7aaac8;font-weight:bold;font-size:13px;'>✅ تلقائي</span>", unsafe_allow_html=True)
                            col_m4.markdown("<span style='color:#6898c0;font-size:12px;'>مدير النظام</span>", unsafe_allow_html=True)
                        else:
                            if has_access:
                                col_m3.markdown("<span style='color:#1dda70;font-weight:bold;font-size:13px;'>✅ مفعّلة</span>", unsafe_allow_html=True)
                                ...
                                col_m3.markdown("<span style='color:#ff6666;font-weight:bold;font-size:13px;'>🚫 محجوبة</span>", unsafe_allow_html=True)
            ...
            with tab_admin_pwd:
                ...
                st.markdown("""
                <div style='background:rgba(80,50,0,0.45);border:2px solid #f9a825;border-radius:10px;padding:14px 18px;margin-bottom:18px;direction:rtl;font-size:14px;'>
                ...
                if df_cwk_users.empty:
                    ...
                    if sel_cwk_user:
                        ...
                        <div style='background:rgba(0,50,20,0.45);border:2px solid #1daa60;border-radius:10px;
                            ...
                            <span style='color:#8aaac8;font-size:13px;'>({sel_cwk_user})</span><br>
                            <span style='font-size:13px;color:#1dda70;'>أمين مستودع المقاول</span>
                        </div>""", unsafe_allow_html=True)
                ...
                if df_arch.empty:
                    ...
                    for _, ar in df_arch.iterrows():
                        ...
                        with st.expander(f"📄 {ar['invoice_type']} | رقم: {ar['invoice_no']} | {str(ar['timestamp'])[:16]} | {ar['employee']}"):
                            st.markdown(f"""
                            <div style='background:rgba(0,20,60,0.60);border-right:4px solid #004a99;border-radius:8px;padding:10px 14px;direction:rtl;font-size:13px;'>
            ...
            with _arch_tab1:
                ...
                if df_af.empty:
                    ...
                    for _, ar in df_af.iterrows():
                        ...
                        with st.expander(f"📄 {ar['invoice_type']} | رقم: {ar['invoice_no']} | {str(ar['timestamp'])[:16]} | {ar['employee']}"):
                            st.markdown(f"""
                            <div style='background:rgba(0,20,60,0.60);border-right:4px solid {_type_color};border-radius:8px;padding:10px 14px;direction:rtl;font-size:13px;'>
                            📍 من: <b>{ar.get('warehouse_from','—')}</b>
                            ...
                            if st.session_state.get(_vk, False):
                                _hr = pd.read_sql("SELECT html_content FROM archived_invoices WHERE id=?", conn, params=(_aid,))
                                if not _hr.empty:
                                    components.html(str(_hr.iloc[0]['html_content']), height=520, scrolling=True)
                ...
                if df_ca.empty:
                    ...
                    for _, ca in df_ca.iterrows():
                        ...
                        with st.expander(f"🚫 فاتورة: {ca['invoice_no']} | الحالة: {ca['status']} | {str(ca['timestamp'])[:16]}"):
                            st.markdown(f"""
                            <div style='background:rgba(60,0,0,0.40);border-right:4px solid {_sc};border-radius:8px;padding:10px 14px;direction:rtl;font-size:13px;'>
                ...
                if df_mine2.empty:
                    ...
                    for _, mr2 in df_mine2.iterrows():
                        ...
                        with st.expander(f"📄 {mr2['invoice_type']} | رقم: {mr2['invoice_no']} | {str(mr2['timestamp'])[:16]}"):
                            st.markdown(f"""
                            <div style='background:rgba(0,20,60,0.60);border-right:4px solid {_t2_color};border-radius:8px;padding:10px 14px;direction:rtl;font-size:13px;'>
                            ...
                            if st.session_state.get(_mr2_vk, False):
                                ...
                                if not _mr2_html.empty:
                                    components.html(str(_mr2_html.iloc[0]['html_content']), height=520, scrolling=True)
                    ...
                    if df_all_entity.empty:
                        ...
                        for _, ent_row in df_all_entity.iterrows():
                            ...
                            with st.expander(f"📄 {ent_row['invoice_type']} | رقم: {ent_row['invoice_no']} | {str(ent_row['timestamp'])[:16]}"):
                                st.markdown(f"""
                                <div style='background:rgba(0,20,60,0.60);border-right:4px solid {_ent_tc};border-radius:8px;padding:10px 14px;direction:rtl;font-size:13px;'>
                                ...
                                if st.session_state.get(_ent_vk, False):
                                    ...
                                    if not _ent_html.empty:
                                        components.html(str(_ent_html.iloc[0]['html_content']), height=520, scrolling=True)
                    st.divider()
                    ...
                    for _, bqrow in df_boq.iterrows():
                        ...
                        with st.expander(
                            ...
                            <div style='background:rgba(0,20,60,0.60);border-right:5px solid {_sc};border-radius:10px;
                                ...
                                <div style='font-size:13px;color:#c8dff4;line-height:2;'>
            ...
            with tab_boq_search:
                ...
                if _manual_boq:
                    ...
                    if df_manual.empty:
                        ...
                        for _, mr in df_manual2.iterrows():
                            ...
                            with st.expander(f"📄 {mr['invoice_type']} | رقم: {mr['invoice_no']} | BOQ: {mr['boq']}"):
                                st.markdown(f"""
                                <div style='background:rgba(3,10,28,0.70);border-right:4px solid {_msc};border-radius:8px;padding:12px 16px;direction:rtl;font-size:13px;'>
                                    📋 <b>رقم BOQ:</b> <span style='color:#7aaac8;font-size:15px;font-weight:900;'>{mr['boq']}</span><br>
                    ...
                    <div style='background:rgba(0,20,60,0.60);border:2px dashed #004a99;border-radius:12px;padding:30px;
                        ...
                        <div style='font-size:15px;font-weight:700;color:#7aaac8;'>اكتب رقم BOQ للبحث</div>
        ...
        if role != "أمين مستودع المقاول":
            ...
            try:
                ...
                for _rct in [("صرف", _rc1, "#e53e3e"), ("ارجاع", _rc2, "#2b6cb0"), ("نقل", _rc3, "#276749")]:
                    ...
                    <div style='background:rgba(3,10,28,0.82);border:2px solid {_rct[2]};border-radius:10px;
                        ...
                        <div style='font-size:12px;color:#8aaac8;'>فاتورة {_rct[0]} مُعادة</div>
                    </div>""", unsafe_allow_html=True)
            except Exception:
                pass
            ...
            if df_cwkr.empty:
                ...
                <div style='background:rgba(0,50,20,0.35);border:2px dashed rgba(29,218,112,0.45);border-radius:14px;
                    ...
                    <div style='font-size:13px;color:#7aaac8;margin-top:6px;'>جميع الفواتير المُرفقة اجتازت المراجعة بنجاح.</div>
                </div>""", unsafe_allow_html=True)
            else:
                    ...
                    with st.expander(
                        ...
                        if str(rtr.get('admin_notes','')).strip():
                            ...
                            _rtr_note_part = (
                                ...
                                f"<span style='color:#ffaa66;font-weight:bold;font-size:15px;'>{_nt}</span>"
                                "</div>"
                            )
                        _rtr_card_html = (
                            ...
                            "padding:16px 20px;direction:rtl;font-size:14px;margin-bottom:12px;'>"
                            ...
                            f"padding:3px 14px;font-size:12px;font-weight:bold;'>{rtr['invoice_type']}</span>"
                            "</div>"
                        ...
                        if _uploaded_rtr is not None:
                            ...
                            st.markdown(
                                ...
                                f"padding:10px 14px;direction:rtl;font-size:13px;margin:8px 0;'>"
                                ...
                                if _cy.button("✅ نعم، أرسل", key=f"rtr_yes_{_rtr_id}"):
                                    ...
                                    if _new_boq and _arch_id_rtr:
                                        ...
                                        if not _old_html_rtr.empty:
                                            ...
                                            _boq_tag = f"<div style='font-size:13px;color:#111111;font-weight:bold;margin-top:4px;'>BOQ الحالة: {_new_boq}</div>"
                                            import re as _re
                                            if "BOQ الحالة:" in _html_rtr:
                                                _html_rtr = _re.sub(r"BOQ الحالة: [^<]*", f"BOQ الحالة: {_new_boq}", _html_rtr)
                                            else:
                                                _html_rtr = _html_rtr.replace(
                                                    f"<span style=\"color:red;font-weight:bold;font-size:16px;\">{rtr['invoice_no']}</span>",
                                                    f"<span style=\"color:red;font-weight:bold;font-size:16px;\">{rtr['invoice_no']}</span>{_boq_tag}"
                                                )
        ...
        if role != "أمين مستودع المقاول":
            ...
            if not _latest_cwk.empty:
                ...
                <div style='background:linear-gradient(135deg,#f0f7ff,#e8f4ff);border:2px solid {_type_color};
                    ...
                    <div style='font-size:13px;font-weight:900;color:{_type_color};margin-bottom:6px;'>🆕 آخر فاتورة {_cwk_inv_type} مُرفقة</div>
                    <div style='font-size:20px;font-weight:900;color:#ff6666;'>رقم: {_lr['invoice_no']}</div>
                    <div style='font-size:13px;color:#8aaac8;margin-top:4px;'>
                        📅 {str(_lr['signed_at'])[:16]} &nbsp;|&nbsp;
                        <span style='background:{_lsc};color:white;border-radius:6px;padding:2px 10px;font-size:12px;'>{_lr['status']}</span>
                ...
                if st.session_state.get(_latest_cwk_vk, False):
                    ...
                    if not _lh.empty:
                        components.html(str(_lh.iloc[0]['html_content']), height=520, scrolling=True)
                ...
                for _, cr in _cwk_all.iterrows():
                    ...
                    with st.expander(f"📄 {cr['invoice_no']} | {str(cr['signed_at'])[:16]} | {cr['status']}"):
                        ...
                        <div style='background:rgba(3,10,28,0.70);border-right:4px solid {_csc};border-radius:8px;
                            ...
                            <span style='background:{_csc};color:white;border-radius:6px;padding:2px 10px;font-size:12px;'>{cr['status']}</span>
                        </div>""", unsafe_allow_html=True)
                        ...
                        if st.session_state.get(_cwk_vk, False):
                            if cr.get('arch_id'):
                                ...
                                if not _cwk_html.empty:
                                    components.html(str(_cwk_html.iloc[0]['html_content']), height=500, scrolling=True)
            ...
            if not st.session_state.get('backup_auth'):
                ...
                <div style='background:linear-gradient(135deg,rgba(0,25,70,0.60),rgba(0,20,60,0.50));border:2px solid #1565c0;
                    ...
                    <div style='display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;'>
                        <div>
                            <div style='font-size:16px;font-weight:900;color:#7aaac8;margin-bottom:6px;'>
                                ⏰ جدول النسخ الاحتياطي التلقائي
                            </div>
                            <div style='font-size:13px;color:#8aaac8;line-height:2;'>
                        ...
                        <div style='text-align:center;background:rgba(3,10,28,0.82);border-radius:10px;padding:12px 20px;'>
                            <div style='font-size:11px;color:#6898c0;'>النسخة القادمة</div>
                            <div style='font-size:17px;font-weight:900;color:#7aaac8;'>{_next_backup}</div>
                            <div style='font-size:11px;color:#6898c0;'>الوقت الحالي: {now_mecca().strftime("%H:%M")}</div>
                        </div>
                    </div>
                </div>""", unsafe_allow_html=True)
                ...
                if df_backups.empty:
                    ...
                    for _, br in df_backups.iterrows():
                        ...
                        _bc1.markdown(
                            f"<div style='direction:rtl;font-size:13px;'>"
                            f"<b>💾 {br['backup_name']}</b><br>"
                            f"<span style='background:{_btype_c};color:white;border-radius:10px;"
                            f"padding:1px 8px;font-size:11px;'>{_btype_l}</span>"
            ...
            if not _allowed_wh:
                ...
                if not df_cwk_inv.empty:
                    ...
                    for _, row in df_cwk_inv.iterrows():
                        ...
                        <tr>
                            <td style='font-weight:900;color:#7aaac8;font-size:12px;padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);'>{row['كود المادة']}</td>
                            <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:13px;'>{row['اسم المادة']}</td>
                            <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);color:#8aaac8;font-size:12px;'>{row['الفئة']}</td>
                            <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);font-size:12px;font-weight:bold;color:#c8dff4;'>📍 {row['المستودع']}</td>
                            <td style='padding:9px 12px;border-bottom:1px solid rgba(0,140,255,0.10);text-align:center;'>
                    ...
                    tbody tr:nth-child(even){{background:#f7faff;}}
                    tbody tr:hover{{background:#e8f0fe;}}
                    .info{{display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:rgba(0,20,60,0.60);border-bottom:1px solid #dde6f5;font-size:13px;color:#8aaac8;direction:rtl;}}
                    </style></head><body>
                    <div class="info"><span>📦 الأصناف: <b style="color:#7aaac8;font-size:15px;">{len(df_cwk_inv)}</b></span>
                    <span style="font-size:11px;color:#6898c0;">🔴 حرج ≤5 &nbsp; 🟡 تنبيه ≤15 &nbsp; 🔵 آمن</span></div>
                    <div class="wrap"><table>
                ...
                if _focused_id is not None:
                    ...
                    if _focused_rows.empty:
                        ...
                        <div style='background:linear-gradient(90deg,#0a1628,#0d2347);border-radius:10px;
                            ...
                            <span style='color:#e8f0ff;font-size:14px;font-weight:900;'>
                                📄 عرض فاتورة: <span style='color:#f9a825;font-size:18px;'>{inv_no}</span>
                            </span>
                            <span style='color:#8eaacc;font-size:12px;'>
                        ...
                        # ── رأس الفاتورة ──
                        _badge_map = {
                            "لم يُرفق":           ("<span style='background:#9e9e9e;color:white;border-radius:8px;padding:3px 12px;font-size:13px;'>📝 لم يُرفق بعد</span>",          False),
                            "بانتظار الاعتماد":   ("<span style='background:#0288d1;color:white;border-radius:8px;padding:3px 12px;font-size:13px;'>🔄 أُرسلت للاعتماد</span>",       False),
                            "معتمد":              ("<span style='background:#1daa60;color:white;border-radius:8px;padding:3px 12px;font-size:13px;'>✅ معتمدة ومخصومة</span>",       False),
                            "مرفوض":              ("<span style='background:#d32f2f;color:white;border-radius:8px;padding:3px 12px;font-size:13px;'>❌ مرفوضة</span>",               False),
                            "مُعادة":             ("<span style='background:#e65100;color:white;border-radius:8px;padding:3px 12px;font-size:13px;'>🔴 مُعادة — يلزم تعديل</span>", True),
                        }
                        ...
                        if _sd is not None and str(_sd.get('admin_notes','')).strip():
                            ...
                            _notes_html = (
                                ...
                                f"<span style='color:#c8dff4;font-size:14px;'>{_nt2}</span><br>"
                                f"<small style='color:#6898c0;'>👤 {_rv2} | 📅 {_ra2}</small>"
                                "</div>"
                            )

                        _sd_sign_html = f"<div style='font-size:13px;color:#8aaac8;margin-top:6px;'>✍️ أرفقها: <b>{_sd['signed_by']}</b> | 📅 {str(_sd['signed_at'])[:16]}</div>" if _sd is not None else ""
                        ...
                        _focus_card = (
                            ...
                            "<div style='font-size:14px;color:#c8dff4;line-height:2;'>"
                        ...
                        components.html(str(inv_row.get('html_content','')), height=520, scrolling=True)
                            ...
                            if _uploaded is not None:
                                ...
                                if _inv_type != "نقل":
                                    ...
                                    <div style='background:rgba(60,50,0,0.40);border-right:3px solid #f9a825;border-radius:6px;
                                        padding:6px 12px;direction:rtl;font-size:12px;color:#8aaac8;margin-bottom:8px;'>
                                ...
                                if st.button(f"📨 إرسال للاعتماد", key=f"send_approval_{inv_id}",
                                    ...
                                    if _inv_type != "نقل" and not _boq_val:
                                        ...
                                        if _boq_val:
                                            ...
                                            _boq_inject = f"<div style='font-size:13px;color:#111111;font-weight:bold;margin-top:4px;'>BOQ الحالة: {_boq_val}</div>"
                                            # أضف BOQ فقط إذا لم يكن موجوداً مسبقاً
                                            if "BOQ الحالة:" not in _old_html:
                                                _new_html = _old_html.replace(
                                                    f"<span style=\"color:red;font-weight:bold;font-size:16px;\">{inv_no}</span>",
                                                    f"<span style=\"color:red;font-weight:bold;font-size:16px;\">{inv_no}</span>{_boq_inject}"
                                                )
                                            else:
                    ...
                    for _, inv_row in df_cwk_list.iterrows():
                        ...
                        if _is_returned and _sd is not None and str(_sd.get('admin_notes','')).strip():
                            ...
                            _note_div = (
                                ...
                                f"font-size:12px;direction:rtl;'>💬 {_nd}</div>"
                            )
                        ...
                        _card_html = (
                            ...
                            f"padding:2px 10px;font-size:12px;'>{_bl}</span></div>"
                            f"<span style='font-size:12px;color:#6898c0;'>📅 {_ts_txt}</span>"
                            "</div>"
                            f"<div style='font-size:13px;color:#8aaac8;margin-top:8px;'>"
                    ...
                    with st.expander(
                        ...
                        with _col_orig:
                            ...
                            _orig_card   = (
                                ...
                                f"padding:1px 8px;font-size:11px;font-weight:bold;margin-right:6px;'>{_ar_type}</span><br>"
                                f"<span style='font-size:13px;color:#c8dff4;line-height:2;'>"
                            ...
                            if st.session_state.get(_vok, False):
                                ...
                                if _orig_html:
                                    components.html(_orig_html, height=460, scrolling=True)
                        ...
                        with _col_sign:
                            ...
                            _sign_card   = (
                                ...
                                f"padding:2px 10px;font-size:12px;font-weight:bold;margin-right:6px;'>{_ar_status}</span><br>"
                                f"<span style='font-size:13px;color:#c8dff4;line-height:2;'>"
                        ...
                        if _ar_status == "معتمد":
                            ...
                            st.markdown(
                                ...
                                "border-radius:8px;padding:10px 14px;direction:rtl;font-size:13px;'>"
