...
""", height=90, scrolling=False)
        ...
        # ── أمين مستودع المقاول: رصيد المستودعات المصرح بها فقط ──
            ...
            if st.button("📊 رصيد المستودعات (المصرح بها)", key="cwk_inv_btn"):
                st.session_state.page = "inventory_status"
                st.session_state["cwk_wh_filter"] = _allowed_wh
                st.query_params["_pg"] = "inventory_status"
                if st.session_state.get("user_info"): st.query_params["_u"] = st.session_state.user_info.get("username","")
    ...
    if st.session_state.page == "inventory_status":
        ...
        # فلتر خاص بأمين مستودع المقاول — يرى فقط مستودعاته
        _cwk_allowed = st.session_state.pop("cwk_wh_filter", None)
        if role == "أمين مستودع المقاول" and not _cwk_allowed:
            _cwk_perm = pd.read_sql("SELECT warehouse FROM contractor_warehouse_permissions WHERE username=?",
                                    conn, params=(u['username'],))
            _cwk_allowed = _cwk_perm['warehouse'].tolist()
        ...
        # أمين مستودع المقاول: يرى فقط مستودعاته المصرح بها
        if role == "أمين مستودع المقاول" and _cwk_allowed:
            _wh_opts_inv = _cwk_allowed
            wh_filter = col_s3.selectbox("🏢 المستودع:", _wh_opts_inv) if len(_wh_opts_inv) > 1 else _wh_opts_inv[0] if _wh_opts_inv else "الكل"
        else:
        ...
        # تطبيق قيود أمين المستودع
        if role == "أمين مستودع المقاول" and _cwk_allowed:
            _wh_placeholders = ",".join(["?" for _ in _cwk_allowed])
            query += f" AND i.warehouse IN ({_wh_placeholders})"
        if cat_filter != "عرض الكل":
            query += f" AND i.category='{cat_filter}'"
        if wh_filter not in ("الكل", None) and role != "أمين مستودع المقاول":
            query += f" AND i.warehouse='{wh_filter}'"
        elif role == "أمين مستودع المقاول" and wh_filter not in ("الكل", None) and len(_cwk_allowed or []) > 1:
            query += f" AND i.warehouse='{wh_filter}'"
...
""", height=_ret_cart_h, scrolling=len(st.session_state.return_cart)>7)
    ...
    # ---------------------------------------------------------
        ...
        if role == "موجه بلاغات":
            ...
            page_header("📥","طلبات ارجاع المواد وإلغاء الفواتير","مراجعة الطلبات واعتمادها","#004a99")
            ...
            # ════════════════════════════════════
            with _tr1:
                _rr_t1,_rr_t2 = st.tabs([f"⏳ المعلقة ({_ret_cnt})","📋 جميع الطلبات"])

                with _rr_t1:
                    df_rr = pd.read_sql("SELECT * FROM return_requests WHERE status=\'معلق\' ORDER BY id DESC",conn)
                    if df_rr.empty:
                        st.success("✅ لا توجد طلبات ارجاع معلقة.")
                    else:
                        for _,rr in df_rr.iterrows():
                            rr=rr.to_dict()
                            try: rr_items=json.loads(rr.get('items_json','[]'))
                            except: rr_items=[]
                            _rr_ck=f"rr_conf_{rr['id']}"
                            with st.expander(f"🔄 {rr['request_no']} | 👤 {rr['requester']} | 📍 {rr.get('warehouse','')} | 📅 {str(rr.get('timestamp',''))[:16]}",expanded=True):
                                st.markdown(f"""<div style='background:rgba(0,30,80,0.50);border:1px solid #1976d2;border-radius:8px;padding:10px 14px;direction:rtl;color:#ddeeff;'>
                                    📍 <b>المستودع:</b> {rr.get('warehouse','—')} | 🏗️ <b>المقاول:</b> {rr.get('contractor','—')}<br>
                                    📦 <b>المواد:</b> {"، ".join([f"{i.get('name','?')} ({i.get('qty',0)})" for i in rr_items])}
                                    {"<br>📋 <b>BOQ:</b> "+str(rr.get('boq','')) if rr.get('boq') else ""}</div>""",unsafe_allow_html=True)

                                if rr.get('invoice_html'):
                                    _rv=f"rr_prev_{rr['id']}"
                                    if st.button("👁️ معاينة الفاتورة",key=f"rr_pvbtn_{rr['id']}"):
                                        st.session_state[_rv]=not st.session_state.get(_rv,False)
                                    if st.session_state.get(_rv,False):
                                        _rh=str(rr['invoice_html']).replace('background:rgba(3,10,28,0.82)','background:white').replace('background:rgba(3,10,28,0.88)','background:white')
                                        components.html(_rh,height=900,scrolling=True)

                                # اختيار المستودع الهدف
                                _wh_target = st.selectbox("📍 المستودع الذي ستُودع فيه المواد:",list_warehouses,key=f"rr_wht_{rr['id']}")

                                if not st.session_state.get(_rr_ck):
                                    _app_c,_rej_c = st.columns(2)
                                    if _app_c.button("✅ اعتماد الارجاع",key=f"rr_app_{rr['id']}",type="primary"):
                                        st.session_state[_rr_ck]=True; st.rerun()
                                    if _rej_c.button("❌ رفض",key=f"rr_rej_{rr['id']}"):
                                    ...
                                    items_txt="، ".join([f"{i.get('name','?')} ({i.get('qty',0)})" for i in rr_items])
                                    st.warning(f"⚠️ تأكيد: ستُودع المواد في [{_wh_target}]: {items_txt}")
                                    _cy,_cn=st.columns(2)
                                    if _cy.button("✅ نعم تأكيد",key=f"rr_yes_{rr['id']}",type="primary"):
                                        _ts=now_mecca().strftime("%Y-%m-%d %H:%M:%S")
                                        # إضافة المواد للمستودع عند الاعتماد فقط
                                        for _it in rr_items:
                                            c.execute("INSERT INTO inventory (item_code,qty,warehouse,contractor,category) VALUES (?,?,?,?,?)",
                                                      (_it.get('code',''),int(_it.get('qty',0)),_wh_target,rr.get('contractor',''),_it.get('cat','')))
                                        c.execute("UPDATE return_requests SET status=\'معتمد\',approved_by=?,approved_at=?,warehouse=? WHERE id=?",
                                                  (u['full_name'],_ts,_wh_target,int(rr['id'])))
                                        save_log("اعتماد ارجاع","—",0,f"اعتماد طلب ارجاع [{rr['request_no']}] → [{_wh_target}]",u['full_name'])
                                        conn.commit()
                                        st.session_state[_rr_ck]=False
                                        st.rerun()
                                    if _cn.button("❌ إلغاء",key=f"rr_no_{rr['id']}"):
                                        st.session_state[_rr_ck]=False; st.rerun()

                with _rr_t2:
                    df_rr_all=pd.read_sql("SELECT * FROM return_requests ORDER BY id DESC",conn)
                    if df_rr_all.empty:
                        st.info("لا توجد طلبات ارجاع.")
                    else:
                        _sf=st.selectbox("الحالة:",["الكل","معلق","معتمد","مرفوض"],key="rr_sf")
                        if _sf!="الكل": df_rr_all=df_rr_all[df_rr_all['status']==_sf]
                        for _,rr in df_rr_all.iterrows():
                            rr=rr.to_dict()
                            _sc={"معتمد":"#1daa60","مرفوض":"#d32f2f","معلق":"#f9a825"}.get(rr['status'],"#9e9e9e")
                            st.markdown(f"<div style='background:rgba(3,10,28,0.78);border:1px solid rgba(0,140,255,0.15);border-radius:8px;padding:9px 14px;direction:rtl;margin:3px 0;'>🔄 <b style='color:#4db8ff;'>{rr['request_no']}</b> | 👤 {rr.get('requester','')} | 📍 {rr.get('warehouse','')} | <span style='color:{_sc};font-weight:700;'>{rr['status']}</span> | 📅 {str(rr.get('timestamp',''))[:16]}</div>",unsafe_allow_html=True)
            ...
            with _tr2:
                import html as _hml
                _ca_t1,_ca_t2 = st.tabs([f"⏳ المعلقة ({_cancel_cnt})","📋 جميع الطلبات"])

                with _ca_t1:
                    df_ca=pd.read_sql("SELECT * FROM cancel_invoice_requests WHERE status=\'معلق\' ORDER BY id DESC",conn)
                    if df_ca.empty:
                        st.success("✅ لا توجد طلبات إلغاء معلقة.")
                    else:
                        for _,cr in df_ca.iterrows():
                            cr=cr.to_dict()
                            try: cr_items=json.loads(cr.get('items_json','[]'))
                            except: cr_items=[]
                            _ca_ck=f"ca_conf_{cr['id']}"
                            with st.expander(f"🚫 {cr['request_no']} | فاتورة: {cr['invoice_no']} | 👤 {cr['requester']} | 📅 {str(cr.get('timestamp',''))[:16]}",expanded=True):
                                st.markdown(f"""<div style='background:rgba(50,0,0,0.45);border:1px solid #c62828;border-radius:8px;padding:10px 14px;direction:rtl;color:#ddeeff;'>
                                    📄 فاتورة <b style='color:#ff6666;'>{_hml.escape(str(cr['invoice_no']))}</b> ({_hml.escape(str(cr.get('invoice_type','')))}) |
                                    📍 {_hml.escape(str(cr.get('warehouse_return','—')))} | 🏗️ {_hml.escape(str(cr.get('contractor','—')))}<br>
                                    📝 <b>السبب:</b> <span style='color:#ffaa66;'>{_hml.escape(str(cr.get('cancel_reason','—')))}</span><br>
                                    📦 <b>المواد:</b> {"، ".join([f"{i.get('name','?')} ({i.get('qty',0)})" for i in cr_items])}</div>""",unsafe_allow_html=True)

                                if cr.get('invoice_html'):
                                    _cv=f"ca_prev_{cr['id']}"
                                    if st.button("👁️ معاينة الفاتورة",key=f"ca_pvbtn_{cr['id']}"):
                                        st.session_state[_cv]=not st.session_state.get(_cv,False)
                                    if st.session_state.get(_cv,False):
                                        ...
                                        components.html(_ch,height=900,scrolling=True)

                                _wh_ret=st.selectbox("📍 مستودع الإيداع بعد الإلغاء:",list_warehouses,key=f"ca_whr_{cr['id']}")

                                if not st.session_state.get(_ca_ck):
                                    _ca1,_ca2=st.columns(2)
                                    if _ca1.button("✅ اعتماد الإلغاء",key=f"ca_app_{cr['id']}",type="primary"):
                                        st.session_state[_ca_ck]=True; st.rerun()
                                    if _ca2.button("❌ رفض",key=f"ca_rej_{cr['id']}"):
                                    ...
                                    items_txt="، ".join([f"{i.get('name','?')} ({i.get('qty',0)})" for i in cr_items])
                                    st.warning(f"⚠️ تأكيد: ستُودع مواد فاتورة {cr['invoice_no']} في [{_wh_ret}]: {items_txt}")
                                    _cy2,_cn2=st.columns(2)
                                    if _cy2.button("✅ نعم تأكيد",key=f"ca_yes_{cr['id']}",type="primary"):
                                        _ts2=now_mecca().strftime("%Y-%m-%d %H:%M:%S")
                                        # إضافة المواد للمستودع عند الاعتماد فقط
                                        for _fi in cr_items:
                                            c.execute("INSERT INTO inventory (item_code,qty,warehouse,contractor,category) VALUES (?,?,?,?,?)",
                                                      (_fi.get('code',''),int(_fi.get('qty',0)),_wh_ret,cr.get('contractor',''),_fi.get('cat','')))
                                        c.execute("UPDATE cancel_invoice_requests SET status=\'معتمد\',approved_by=?,approved_at=? WHERE id=?",
                                                  (u['full_name'],_ts2,int(cr['id'])))
                                        save_log("اعتماد إلغاء فاتورة","—",0,f"اعتماد إلغاء [{cr['invoice_no']}] → [{_wh_ret}]",u['full_name'])
                                        conn.commit()
                                        st.session_state[_ca_ck]=False
                                        st.rerun()
                                    if _cn2.button("❌ إلغاء",key=f"ca_no_{cr['id']}"):
                                        st.session_state[_ca_ck]=False; st.rerun()

                with _ca_t2:
                    df_ca_all=pd.read_sql("SELECT * FROM cancel_invoice_requests ORDER BY id DESC",conn)
                    if df_ca_all.empty:
                        st.info("لا توجد طلبات إلغاء.")
                    else:
                        _scf=st.selectbox("الحالة:",["الكل","معلق","معتمد","مرفوض"],key="ca_scf")
                        if _scf!="الكل": df_ca_all=df_ca_all[df_ca_all['status']==_scf]
                        for _,cr in df_ca_all.iterrows():
                            cr=cr.to_dict()
                            _sc2={"معتمد":"#1daa60","مرفوض":"#d32f2f","معلق":"#f9a825"}.get(cr['status'],"#9e9e9e")
                            st.markdown(f"<div style='background:rgba(3,10,28,0.78);border:1px solid rgba(0,140,255,0.15);border-radius:8px;padding:9px 14px;direction:rtl;margin:3px 0;'>🚫 <b style='color:#ff6666;'>{cr['request_no']}</b> | {cr['invoice_no']} | 👤 {cr.get('requester','')} | <span style='color:{_sc2};font-weight:700;'>{cr['status']}</span> | 📅 {str(cr.get('timestamp',''))[:16]}</div>",unsafe_allow_html=True)
    ...
    # ---------------------------------------------------------
        ...
        if role not in ("مدير نظام","مسؤول المستودعات","موجه بلاغات"):
        ...
        if role == "موجه بلاغات":
            ...
            _ef_emp  = _ef_f1.selectbox("👤 الموجه:",_ef_emps,key="ef_emp")
            _ef_date = _ef_f2.date_input("📅 من تاريخ:",value=None,key="ef_date")
            _ef_no   = _ef_f3.text_input("🔍 رقم الفاتورة:",key="ef_no",placeholder="G1...").strip()
            _efq = "SELECT id,invoice_no,invoice_type,warehouse_from,contractor,employee,timestamp FROM archived_invoices WHERE 1=1"
            if _ef_emp!="الكل": _efq+=f" AND employee=\'{_ef_emp}\'"
            if _ef_date:         _efq+=f" AND DATE(timestamp)>=\'{_ef_date.strftime('%Y-%m-%d')}\'"
            if _ef_no:           _efq+=f" AND invoice_no LIKE \'%{_ef_no}%\'"
            _efq+=" ORDER BY id DESC LIMIT 100"
            _ef_df = pd.read_sql(_efq,conn)

        # ── عرض القائمة ──
        if not st.session_state.get('ef_selected'):
            if _ef_df.empty:
                st.info("لا توجد فواتير.")
            else:
                last_id = int(_ef_df.iloc[0]['id'])
                for _,r2 in _ef_df.iterrows():
                    r2=r2.to_dict()
                    _tc={"صرف":"#e53e3e","ارجاع":"#2b6cb0","نقل":"#276749"}.get(r2['invoice_type'],"#555")
                    _is_last=(int(r2['id'])==last_id)
                    st.markdown(f"""<div style='background:{"rgba(60,40,0,0.40)" if _is_last else "rgba(3,10,28,0.75)"};border:{"2px solid #f9a825" if _is_last else "1px solid rgba(0,140,255,0.15)"};border-radius:10px;padding:10px 14px;direction:rtl;margin:4px 0;'>
                    <span style='background:{_tc};color:white;border-radius:4px;padding:1px 7px;font-size:13px;'>{r2['invoice_type']}</span> <b style='color:#ddeeff;'>{r2['invoice_no']}</b>{"&nbsp;<span style='background:#f9a825;color:#111;border-radius:4px;padding:1px 6px;font-size:12px;'>⭐</span>" if _is_last else ""}
                    <span style='color:#6898c0;font-size:13px;float:left;'>📅 {str(r2.get('timestamp',''))[:16]}</span><br>
                    <span style='color:#8aaac8;font-size:13px;'>{"👤 "+str(r2.get('employee',''))+" | " if role!="موجه بلاغات" else ""}📍 {r2.get('warehouse_from','—')} | 🏗️ {r2.get('contractor','—')}</span></div>""",unsafe_allow_html=True)
                    if st.button(f"✏️ تعديل {r2['invoice_no']}",key=f"ef_pick_{r2['id']}"):
                        st.session_state['ef_selected'] = r2['invoice_no']
                        st.session_state['ef_items'] = None
                        st.session_state['ef_compare'] = False
                        st.rerun()

        # ── شاشة التعديل ──
        else:
            _inv_no = st.session_state['ef_selected']
            _inv_df = pd.read_sql(f"SELECT * FROM archived_invoices WHERE invoice_no=\'{_inv_no}\'",conn)
            if _inv_df.empty:
                st.error("❌ الفاتورة غير موجودة."); st.session_state.pop('ef_selected',None); st.rerun()
            _inv = dict(_inv_df.iloc[0])

            if st.button("✖️ رجوع للقائمة",key="ef_back_list"):
                for k in ['ef_selected','ef_items','ef_compare','ef_new_wh','ef_new_cont']: st.session_state.pop(k,None)
                st.rerun()

            st.markdown(f"<div style='background:rgba(0,30,80,0.50);border:1px solid rgba(0,140,255,0.25);border-radius:10px;padding:10px 14px;direction:rtl;margin-bottom:12px;'>✏️ تعديل فاتورة <b style='color:#4db8ff;'>{_inv_no}</b> | {_inv['invoice_type']} | 👤 {_inv.get('employee','')}</div>",unsafe_allow_html=True)

            if st.session_state.get('ef_items') is None:
                try: st.session_state['ef_items']=json.loads(_inv.get('items_json','[]') or '[]')
                except: st.session_state['ef_items']=[]

            if not st.session_state.get('ef_compare'):
                # ── نموذج التعديل ──
                _whs = list_warehouses or [_inv.get('warehouse_from','')]
                _cos = list_contractors or [_inv.get('contractor','')]
                _wh_i  = _whs.index(_inv.get('warehouse_from','')) if _inv.get('warehouse_from','') in _whs else 0
                _co_i  = _cos.index(_inv.get('contractor',''))    if _inv.get('contractor','')    in _cos else 0
                ...
                for _ii,_it in enumerate(st.session_state['ef_items']):
                    _col1,_col2,_col3=st.columns([3,1,0.5])
                    _col1.markdown(f"<span style='color:#ddeeff;'><b style='color:#7aaac8;'>{_it.get('code','')}</b> — {_it.get('name','')}</span>",unsafe_allow_html=True)
                    _q2=_col2.number_input("ك",min_value=0,value=int(_it.get('qty',1)),key=f"ef_q2_{_ii}",label_visibility="collapsed")
                    _d2=_col3.button("🗑️",key=f"ef_d2_{_ii}")
                    if not _d2 and _q2>0: _items2.append({**_it,'qty':_q2})
                st.session_state['ef_items']=_items2

                st.divider()
                st.markdown("**➕ إضافة مادة:**")
                _a1,_a2,_a3=st.columns([2,1,1])
                _acode=_a1.text_input("كود:",key="ef_acode",placeholder="908514012").strip()
                ...
                if st.checkbox("👁️ معاينة الفاتورة الجديدة مباشرة",key="ef_lprev"):
                    _lh=render_invoice_html("فاتورة صرف مواد طوارئ",st.session_state['ef_items'],_nwh,_ncont,_inv['employee'],_inv_no,boq=_inv.get('boq',''))
                    _lh=_lh.replace('background:rgba(3,10,28,0.82)','background:white').replace('background:rgba(3,10,28,0.88)','background:white')
                    components.html(_lh,height=900,scrolling=True)

                st.divider()
                if st.button("🔍 مقارنة القديم والجديد ثم تأكيد",key="ef_tocmp",type="primary",use_container_width=True):
                    st.session_state['ef_new_wh']  =_nwh
                    st.session_state['ef_new_cont']=_ncont
                    st.session_state['ef_compare'] =True
                    st.rerun()

            else:
                # ── شاشة المقارنة ──
                _nwh  =st.session_state.get('ef_new_wh',  _inv.get('warehouse_from',''))
                _ncont=st.session_state.get('ef_new_cont',_inv.get('contractor',''))
                _items2=st.session_state.get('ef_items',[])
                _c1,_c2=st.columns(2)
                _c1.markdown("<div style='text-align:center;color:#ff6666;font-weight:900;'>🔴 القديمة</div>",unsafe_allow_html=True)
                _c2.markdown("<div style='text-align:center;color:#1dda70;font-weight:900;'>🟢 الجديدة</div>",unsafe_allow_html=True)
                _oh=str(_inv.get('html_content',''))
                _oh=_oh.replace('background:rgba(3,10,28,0.82)','background:white').replace('background:rgba(3,10,28,0.88)','background:white')
                with _c1: components.html(_oh,height=800,scrolling=True)
                _nh=render_invoice_html("فاتورة صرف مواد طوارئ",_items2,_nwh,_ncont,_inv['employee'],_inv_no,boq=_inv.get('boq',''))
                _nhs=_nh.replace('background:rgba(3,10,28,0.82)','background:white').replace('background:rgba(3,10,28,0.88)','background:white')
                with _c2: components.html(_nhs,height=800,scrolling=True)
                st.divider()
    ...
    # ---------------------------------------------------------
        ...
        if u['role'] not in ("مدير نظام", "مسؤول المستودعات"):
            ...
            with _tab_stock:
                ...
                # ── فلاتر في الأعلى ──
                _af1, _af2, _af3, _af4 = st.columns([2, 1.5, 1.5, 1.5])
                ...
                _mb_inv_search = _af4.text_input("🔍 رقم الفاتورة:", key="mb_inv_search", placeholder="G1, G2...").strip()
                ...
                if _mb_inv_search:
                    _mb_query += " AND a.invoice_no LIKE ?"
                    _mb_params.append(f"%{_mb_inv_search}%")
