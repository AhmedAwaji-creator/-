...
def init_database():
    ...
    # جدول طلبات الارتجاع (من موجه البلاغات — يحتاج اعتماد مسؤول المستودع أو مدير النظام)
...
# =========================================================
    ...
    # ---------------------------------------------------------
    # صفحة: تقديم طلب ارتجاع (موجه البلاغات)
    # ---------------------------------------------------------
    elif st.session_state.page == "return_requests_user":
        st.markdown("<div class='main-title'>📤 تقديم طلب ارتجاع مواد</div>", unsafe_allow_html=True)
        if not list_warehouses or not list_contractors:
            ...
            tab_new_req, tab_my_req = st.tabs(["➕ طلب ارتجاع جديد", "📋 طلباتي السابقة"])
            ...
            with tab_new_req:
                ...
                if st.session_state.ret_req_cart:
                    ...
                    if st.session_state.ret_req_review:
                        st.write("---")
                        req_no_preview = "RR" + datetime.now().strftime("%d%H%M")
                        html_rr = render_return_invoice_html("فاتورة ارتجاع مواد طوارئ — بانتظار الاعتماد",
                        ...
                        st.markdown(f"""<div class='warn-box'>⚠️ <b>سيتم رفع طلب الارتجاع التالي للاعتماد:</b><br>
                        ...
                        if col_send.button("🚀 إرسال الطلب للاعتماد"):
                            ...
                            conn.commit()
                            save_log("طلب ارتجاع جديد", "—", 0,
                                     f"تقديم طلب ارتجاع رقم [{req_no_preview}] للمستودع [{rr_wh}] من المقاول [{rr_contractor}]",
                                     u['full_name'])
                            st.session_state.ret_req_cart = []; st.session_state.ret_req_review = False
                            st.success(f"✅ تم إرسال طلب الارتجاع رقم ({req_no_preview}) بنجاح وهو الآن بانتظار الاعتماد!"); st.rerun()
                        if col_cancel.button("❌ إلغاء"):
                            st.session_state.ret_req_review = False; st.rerun()

            with tab_my_req:
                st.write(f"### 📋 طلبات الارتجاع المقدمة من: {u['full_name']}")
                df_my_req = pd.read_sql(
                    f"SELECT * FROM return_requests WHERE requester='{u['full_name']}' ORDER BY id DESC", conn)
                if df_my_req.empty:
                    st.info("ℹ️ لم تقم بتقديم أي طلبات ارتجاع حتى الآن.")
                else:
                    for _, rr in df_my_req.iterrows():
                        ...
                        st.markdown(f"""<div class='report-box'>
                            📄 <b>طلب ارتجاع رقم (<span style='color:red;'>{rr['request_no']}</span>)</b>
                ...
                if df_pending.empty:
                    ...
                    for _, rr in df_pending.iterrows():
                        ...
                        with st.expander(f"📄 طلب رقم {rr['request_no']} | من: {rr['requester']} | {rr['timestamp']}", expanded=True):
                            ...
                            st.write("##### ✏️ المواد المطلوب ارتجاعها (يمكن تعديل الكميات أو حذف صنف):")
                            ...
                            if st.session_state.view_archived_html.get(f"prev_{rr['id']}", False):
                                ...
                                html_prev = render_return_invoice_html("فاتورة ارتجاع مواد طوارئ — معتمدة",
                            ...
                            if btn_col2.button(f"❌ رفض الطلب", key=f"reject_{rr['id']}"):
                                c.execute("UPDATE return_requests SET status='مرفوض', approved_by=?, approved_at=? WHERE id=?",
                                          (u['full_name'], datetime.now().strftime("%Y-%m-%d %H:%M:%S"), int(rr['id'])))
                                conn.commit()
                                save_log("رفض طلب ارتجاع", "—", 0, f"رفض طلب الارتجاع رقم [{rr['request_no']}]", u['full_name'])
                                st.warning(f"⚠️ تم رفض طلب الارتجاع ({rr['request_no']})."); st.rerun()
                            st.markdown("</div>", unsafe_allow_html=True)
                            ...
                            if st.session_state.get(adm_confirm_key):
                                ...
                                st.markdown(f"""<div class='warn-box'>⚠️ <b>تأكيد اعتماد طلب الارتجاع ({rr['request_no']})؟</b><br>
                                ...
                                if cy.button(f"✅ نعم، اعتماد وإضافة للمستودع", key=f"conf_yes_{rr['id']}"):
                                    ...
                                    for fitem in final_items:
                                        ...
                                        save_log("اعتماد ارتجاع مواد", fitem['code'], fitem['qty'],
                                                 f"اعتماد ارتجاع طلب [{final_inv_no}] إلى مستودع [{final_wh}] من [{rr['contractor']}]",
                                                 u['full_name'])
                                    new_html = render_return_invoice_html("فاتورة ارتجاع مواد طوارئ — معتمدة",
                                                                           final_items, final_wh, rr['contractor'],
                                                                           u['full_name'], final_inv_no)
                                    archive_invoice("ارتجاع", final_inv_no, final_wh, "", rr['contractor'],
                                    ...
                                    conn.commit()
                                    st.success(f"🎉 تم اعتماد طلب الارتجاع ({final_inv_no}) وإضافة المواد إلى مستودع [{final_wh}] بنجاح!"); st.rerun()
                        ...
                        for ni in new_items:
                            if not any(x['code']==ni['code'] for x in orig_items):
                                ...
                                if inv_type == "صرف":
                                    ...
                                    c.execute("INSERT INTO inventory (item_code,qty,warehouse,contractor,category) VALUES (?,?,?,?,?)",
                                              (ni['code'], int(ni['qty']), wh_from, contractor, cat))
                                    save_log("تعديل فاتورة ارتجاع - إضافة صنف", ni['code'], ni['qty'],
                                             f"إضافة صنف [{ni['name']}] للفاتورة ({row['invoice_no']})", u['full_name'])
        ...
        if role == "مدير نظام":
            ...
            with adm_tab1:
                ...
                with st.form("add_contact_form", clear_on_submit=True):
                    ...
                    cb1, cb2 = st.columns([1.5, 1.5])
                    ct_etype = cb1.selectbox("نوع الجهة *", ["مقاول", "مستودع"])
                    ct_pos   = cb2.selectbox("المنصب الوظيفي *",
                                             CONTRACTOR_POSITIONS if True else WAREHOUSE_POSITIONS)
                    # نوع الجهة يحدد القائمة
                    if ct_etype == "مقاول":
                        _pos_list = CONTRACTOR_POSITIONS
                        _ename_list = ct_contractors if ct_contractors else ["لا يوجد مقاولون"]
                        ct_pos_sel  = st.selectbox("المنصب *", _pos_list, key="add_pos_sel")
                        ct_ename    = st.selectbox("اسم المقاول (من المقاولين المعتمدين) *", _ename_list)
                    else:
                        _pos_list = WAREHOUSE_POSITIONS
                        _ename_list = ct_warehouses if ct_warehouses else ["لا توجد مستودعات"]
                        ct_pos_sel  = st.selectbox("المنصب *", _pos_list, key="add_pos_sel")
                        ct_ename    = st.selectbox("اسم المستودع (من مستودعات النظام) *", _ename_list)
        ...
        if u['role'] != "مدير نظام":
            ...
            with tab_add_new_emp:
                ...
                with st.form("add_user_form", clear_on_submit=True):
                    ...
                    _pos_opts_new = ["مدير المشروع", "نائب مدير المشروع", "مستلم بلاغات", "أمين مستودع", "مسؤول فرق طوارئ", "أخرى"]
                    new_position = st.selectbox("المنصب الوظيفي (يظهر في دليل أرقام التواصل)", _pos_opts_new)
